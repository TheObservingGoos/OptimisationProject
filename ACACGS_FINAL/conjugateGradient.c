...

{
  ...
  *normr = 0.0;
  ...
  TICK();
  waxpby(nrow, 1.0, x, 0.0, x, p); // Loop Unrolling and OpenMP parallelisation is used here
  TOCK(t2);
  TICK();
  sparsemv(A, p, Ap); // Vectorisation, Loop Unrolling and OpenMP parallelisation is used here
  TOCK(t3);
  TICK();
  waxpby(nrow, 1.0, b, -1.0, Ap, r);
  TOCK(t2);
  TICK();
  ddot(nrow, r, r, &rtrans); // Vectorisation, Loop Unrolling and OpenMP parallelisation is used here
  TOCK(t1);
  *normr = sqrt(rtrans);
